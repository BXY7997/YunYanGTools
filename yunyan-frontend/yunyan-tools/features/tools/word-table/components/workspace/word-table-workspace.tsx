...
import { WordTableFooter } from "@/features/tools/word-table/components/workspace/sections/word-table-footer"
import {
  BorderEditorCard,
  clampInteger,
  countFilledCells,
  sumBorder,
  WordTablePreview,
} from "@/features/tools/word-table/components/workspace/sections/word-table-preview"
import {
  exportWordTableWord,
...
export function WordTableWorkspace({ tool, groupTitle }: WordTableWorkspaceProps) {
  ...
  const { notice, source, savedAt, updateNotice } = useToolNotice({
    ...
    initialText: toolWorkspaceCopy.wordTable.initialNotice,
  ...
  return (
    <ToolWorkspaceShell>
      <ToolWorkspaceHero
        srOnlyTitle="Word表格制作工具 - AI一键生成与手动编辑表格"
        title="Word表格制作"
        subtitle="AI生成表格 / 手动编辑表格"
        description="支持表头与单元格编辑、行列扩展、边框精细设置和 Word 导出，适用于测试文档、项目计划、课程作业与论文附表。"
        tags={["三线表", "普通表格", "论文格式"]}
      />
        ...
        <WordTableFooter showFaq={Boolean(workspaceModules.faqItem)} />
    </ToolWorkspaceShell>
  )
}
