...
import {
  ...
  aigcReduceFaqItems,
  aigcReduceGuideSteps,
  aigcReduceKeywordList,
  aigcReduceModeTabs,
  aigcReducePreviewResult,
  aigcReduceSeoParagraph,
  aigcReduceSplitModes,
} from "@/features/tools/aigc-reduce/constants/aigc-reduce-config"
import { generateAigcReduceData } from "@/features/tools/aigc-reduce/services/aigc-reduce-api"
import type {
  AigcReduceMode,
  AigcReduceResult,
  AigcReduceSplitMode,
...
interface AigcReduceWorkspaceProps {
    ...
    items.push(
      {
        key: "before",
        label: "处理前",
        value: formatPercent(params.result.beforeProbability),
      },
      {
        key: "after",
        label: "处理后",
        value: formatPercent(params.result.afterProbability),
      },
      {
        key: "delta",
        label: "降幅",
        value: formatPercent(
          Math.max(
            0,
            params.result.beforeProbability - params.result.afterProbability
          )
        ),
      }
    )
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <article className="space-y-3 rounded-lg border border-border/70 bg-background/75 p-3">
            <p className="text-sm font-semibold text-foreground">概率变化</p>
            <ProbabilityBar
              label="处理前AIGC概率"
              value={result.beforeProbability}
              barClassName="bg-amber-500"
            />
            <ProbabilityBar
              label="处理后AIGC概率"
              value={result.afterProbability}
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              <div className="rounded-md border border-border/70 bg-card p-2">
                ...
                <p className="mt-1 font-semibold text-foreground">
                  {formatPercent(
                    Math.max(0, result.beforeProbability - result.afterProbability)
                  )}
                </p>
              </div>
              <div className="rounded-md border border-border/70 bg-card p-2">
        ...
        <article className="rounded-lg border border-border/70 bg-background/75 p-3">
          <p className="text-sm font-semibold text-foreground">改写说明</p>
...
export function AigcReduceWorkspace({ tool, groupTitle }: AigcReduceWorkspaceProps) {
  const { setWorkspaceHeaderStatus } = useWorkspaceHeaderStatus()

  const modeDraft = useLocalDraftState<AigcReduceMode>({
    storageKey: "tools:draft:aigc-reduce:mode:v1",
    initialValue: "text",
    schemaVersion: toolDraftSchemaVersions.aigcReduce,
  })
  const splitModeDraft = useLocalDraftState<AigcReduceSplitMode>({
    storageKey: "tools:draft:aigc-reduce:split-mode:v1",
    initialValue: "sentence",
    schemaVersion: toolDraftSchemaVersions.aigcReduce,
  })
  const titleDraft = useLocalDraftState<string>({
    storageKey: "tools:draft:aigc-reduce:title:v1",
    initialValue: aigcReduceDefaultTitle,
    schemaVersion: toolDraftSchemaVersions.aigcReduce,
  })
  const contentDraft = useLocalDraftState<string>({
    storageKey: "tools:draft:aigc-reduce:content:v1",
    initialValue: aigcReduceDefaultContent,
    schemaVersion: toolDraftSchemaVersions.aigcReduce,
  })
  ...
  const [notice, setNotice] = React.useState<NoticeState>({
    tone: "info",
    text: toolWorkspaceCopy.aigcReduce.initialNotice,
  })
  ...
  const [result, setResult] = React.useState<AigcReduceResult | null>(null)
  ...
  const previewResult = result || aigcReducePreviewResult
  ...
  const validateBeforeParse = React.useCallback(() => {
    if (mode === "file") {
      if (!selectedFile) {
        updateNotice("error", toolWorkspaceCopy.aigcReduce.fileRequired)
        return false
      }
      return true
    }
    ...
    if (!content.trim()) {
      updateNotice("error", toolWorkspaceCopy.aigcReduce.contentRequired)
      return false
    }
  ...
  const handleParse = React.useCallback(async () => {
    ...
    setLoading("parse")
    try {
      const generated = await generateAigcReduceData(
        {
          mode,
          splitMode,
          title,
      ...
      updateNotice(
        "success",
        withNoticeDetail(toolWorkspaceCopy.aigcReduce.parseSuccess, generated.message),
        generated.source
      )
    } catch (error) {
  ...
  React.useEffect(() => {
    ...
    setWorkspaceHeaderStatus({
      ...
      saveModeLabel: resolveWorkspaceSourceLabel("aigc-reduce", source),
    })
  }, [
    groupTitle,
    notice.text,
  ...
  return (
    <ToolWorkspaceShell>
      <ToolWorkspaceHero
        srOnlyTitle="在线AIGC降低工具 - 免费论文AI内容优化与学术表达改写"
        title="论文AIGC率降低"
        subtitle="一键降低您的论文AIGC率"
        description="通过句式重组、表达重写和术语规范化降低文本相似风险，当前为前端模拟闭环，后续可平滑接入真实改写服务。"
        tags={["文本改写", "分段策略", "可接入后端"]}
      />
        ...
        <section className={toolsWorkspaceLayout.surfaceSection}>
          {workspaceModules.sectionHeading ? (
            <ToolSectionHeading
              title="开始使用"
              description="选择文件上传或文本输入，执行降AIGC改写并查看优化结果"
            />
          ) : null}
            ...
            <div className="rounded-md border border-border/70 bg-background/75 px-3 py-2">
              <div className="flex flex-wrap items-center gap-3 text-sm">
                ...
                <div className="inline-flex rounded-md border border-border/70 bg-card p-0.5">
                  {aigcReduceSplitModes.map((option) => (
                    <button
                      key={option.value}
                      type="button"
          ...
          <ToolCollapsibleFooter contentClassName={toolsWorkspaceLayout.footerContent}>
            <section className={toolsWorkspaceLayout.footerSection}>
              <h2 className={toolsWorkspaceLayout.footerTitle}>
                在线AIGC率降低工具 - 论文AI内容优化与学术表达改写
              </h2>
              <p className={toolsWorkspaceLayout.footerBody}>{aigcReduceSeoParagraph}</p>
            </section>

            <section className={toolsWorkspaceLayout.footerSection}>
              <h2 className={toolsWorkspaceLayout.footerTitle}>如何使用AIGC率降低工具？</h2>
              <ol className={toolsWorkspaceLayout.footerList}>
                {aigcReduceGuideSteps.map((step) => (
                  <li key={step}>{step}</li>
                ))}
              </ol>
            </section>
