...
import { PseudoCodeFooter } from "@/features/tools/pseudo-code/components/workspace/sections/pseudo-code-footer"
import { PreviewCodeBlock } from "@/features/tools/pseudo-code/components/workspace/sections/pseudo-code-preview-block"
import {
...
export function PseudoCodeWorkspace({ tool, groupTitle }: PseudoCodeWorkspaceProps) {
  ...
  const { notice, source, savedAt, updateNotice } = useToolNotice({
    initialText: toolWorkspaceCopy.pseudoCode.initialNotice,
  ...
  const handleExportImage = React.useCallback(async () => {
    ...
    try {
      ...
      const reason =
        error instanceof Error ? error.message : toolWorkspaceCopy.common.exportFailed
      updateNotice("error", withNoticeDetail(toolWorkspaceCopy.common.exportFailed, reason))
    } finally {
      setLoading(null)
    }
  }, [createController, imageExportFormat, isAbortError, previewDocument, updateNotice])
  ...
  }, [createController, isAbortError, previewDocument, updateNotice])
  ...
  return (
    <ToolWorkspaceShell>
      <ToolWorkspaceHero
        srOnlyTitle="伪代码生成工具 - AI生成与算法可视化排版"
        title="伪代码生成工作台"
        subtitle="AI草拟 + 手动精修 + 论文规范导出"
        description="不是普通输入框，而是一套可配置的算法排版流水线：支持 AI 草拟、手动精修、行号规范、关键字可视化，以及 PNG/SVG/Word 规范导出。"
        tags={["算法排版", "SVG/图片导出", "Word标准格式"]}
      />
        ...
        <section className={toolsWorkspaceLayout.surfaceSection}>
          ...
          <div className={toolsLayoutTokens.workspace.splitLayoutWideClass}>
            ...
            <aside className="xl:sticky xl:top-6">
              <section className="tools-right-grid-pane rounded-xl p-4 md:p-5">
                <Tabs
                  value={rightPanelTab}
                  onValueChange={(value) =>
        ...
        <PseudoCodeFooter showFaq={Boolean(workspaceModules.faqItem)} />
    </ToolWorkspaceShell>
  )
}
